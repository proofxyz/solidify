...
contract FeaturesLoader is Script {
    ...
    string public constant PATH = "./test/features/gen/features.json";
    ...
    function getProof(uint256 tokenId) public view returns (bytes32[] memory) {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            if (odd) {
                // If the node is odd, we the neighbour in the tree that has to
                // be used for hashing is just the one before.
                neighbour = tokenId - 1;
            } else {
                // If the node is even, we need to check if we are at the end of
                // our node.
                if (tokenId != _hashes[i].length - 1) {
                    // if not, we can just use the next node for hashing.
                    neighbour = tokenId + 1;
                } else {
                    // otherwise, the node needs to be hashed with itself.
                    neighbour = tokenId;
                }
            }
